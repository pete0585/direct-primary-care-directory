...
export default async function HomePage() {
    ...
    return (<div>
      ...
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <FeatureCard icon={<DollarSign className="w-6 h-6 text-brand-teal" aria-label="flat fee"/>} title="Flat Monthly Fee" body="Pay $50–$150/month directly to your doctor. No insurance billing, no copays, no deductibles."/>
            <FeatureCard icon={<Clock className="w-6 h-6 text-brand-teal" aria-label="same-day access"/>} title="Same-Day Access" body="DPC doctors see 300–600 patients instead of 2,000+. Same-day appointments are the norm."/>
            <FeatureCard icon={<HeartPulse className="w-6 h-6 text-brand-teal" aria-label="comprehensive care"/>} title="Comprehensive Care" body="Annual exams, labs, acute visits, chronic disease management, telehealth — all in the membership."/>
            <FeatureCard icon={<ShieldCheck className="w-6 h-6 text-brand-teal" aria-label="HSA eligible"/>} title="HSA Eligible" body="Pair DPC with a high-deductible plan + HSA for the most cost-effective healthcare setup."/>
          </div>
        </div>
      </section>

      {/* Featured listings */}
      {featured.length > 0 && (<section className="py-14">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-display text-brand-navy text-xl font-bold uppercase tracking-wide">
                Featured DPC Practices
              </h2>
              <Link href="/listings" className="flex items-center gap-1 text-brand-teal font-body text-sm hover:underline">
      ...
      <section className="py-14 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {HERO_SPECIALTIES.map(s => (<Link key={s} href={`/categories/${s.replace(/_/g, '-')}`} className="bg-surface border border-surface-border hover:border-brand-teal rounded-xl p-4 text-center transition-all hover:shadow-sm group">
      ...
      {/* Browse by state */}
      {topStates.length > 0 && (<section className="py-14">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="font-display text-brand-navy text-xl font-bold uppercase tracking-wide mb-8 text-center">
              Find DPC Practices by State
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
              {topStates.map(({ state, count }) => (<Link key={state} href={`/listings?state=${state}`} className="bg-white border border-surface-border hover:border-brand-teal rounded-xl p-4 text-center transition-all hover:shadow-sm group">
      ...
      <section className="py-14 bg-white">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="font-body text-gray-600 mb-6">
            ...
            here right now.</p>
          <Link href="/submit" className="inline-block bg-brand-teal hover:bg-brand-teal-dark text-white font-body font-semibold px-8 py-3.5 rounded-xl transition-colors">
...
function FeatureCard({ icon, title, body }: {
    ...
    return (<div className="bg-surface border border-surface-border rounded-xl p-5">
...
function CompareCard({ title, color, items }: {
    ...
    return (<div className={`rounded-xl p-5 ${color === 'teal' ? 'bg-brand-teal/10 border border-brand-teal/30' : 'bg-white/5 border border-white/10'}`}>
      <h3 className={`font-display font-bold text-sm uppercase tracking-wide mb-3 ${color === 'teal' ? 'text-brand-teal' : 'text-gray-400'}`}>
        {title}
      </h3>
      <ul className="space-y-2">
        {items.map(item => (<li key={item} className="font-body text-sm text-gray-200 flex items-start gap-2">
