...
export default function ListingCard({ listing }: ListingCardProps) {
    ...
    return (<Link href={`/listings/${listing.slug}`} className="block group">
      <div className={`bg-white rounded-xl border transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 h-full flex flex-col
        ...
        <div className="p-5 flex-1 flex flex-col gap-3">
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-full bg-brand-mint border border-brand-teal/20 flex items-center justify-center shrink-0 overflow-hidden">
              {listing.photo_url ? (<Image src={listing.photo_url} alt={listing.full_name} width={48} height={48} className="w-12 h-12 object-cover"/>) : (<span className="text-brand-navy font-display text-lg font-bold">
            ...
            <div className="min-w-0 flex-1">
              ...
              {listing.practice_name && (<p className="text-gray-500 text-xs font-body mt-0.5 truncate">{listing.practice_name}</p>)}
          ...
          {listing.specialties.length > 0 && (<div className="flex flex-wrap gap-1">
              {listing.specialties.slice(0, 3).map(s => (<span key={s} className="bg-brand-mint text-brand-navy text-xs font-body px-2 py-0.5 rounded-full">
                  {formatSpecialty(s)}
                </span>))}
            </div>)}
          ...
          <div className="flex items-center gap-4 text-xs font-body text-gray-500 mt-auto pt-2 border-t border-surface-border">
            ...
            {listing.telehealth_available && (<div className="flex items-center gap-1">
                <Video className="w-3.5 h-3.5 text-brand-teal" aria-label="telehealth"/>
                <span>Telehealth</span>
              </div>)}
            {listing.accepts_children && (<div className="flex items-center gap-1">
          ...
          <div className="flex items-center justify-between">
            ...
            {listing.accepting_new_patients ? (<span className="flex items-center gap-1 text-xs font-body text-emerald-600">
                <Check className="w-3.5 h-3.5" aria-label="accepting patients"/>
                Accepting Patients
              </span>) : (<span className="text-xs font-body text-gray-400">Waitlist only</span>)}
          </div>
        </div>
      </div>
    </Link>);
